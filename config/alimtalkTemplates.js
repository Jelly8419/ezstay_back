...
const templates = {
  ...
  contract_rejected_guest: {
    ...
    fallbackContent:
      ...
      `방 이름: #{방이름}\n` +
      `계약 기간: #{입주일} ~ #{퇴실일}\n\n` +
      `아쉽게도 회원님이 요청하신 계약을 임대인님이 승인하지 않았어요. 다른 방을 찾아주세요.`,
    buildFallbackSMS: (data) =>
  ...
  option_payment_guest: {
    ...
    varMap: { roomName: '방이름', startDate: '입주일', endDate: '퇴실일', optionItems: '옵션상품목록', amount: '추가결제금액' },
    fallbackContent:
      `[옵션 추가 결제 안내]\n\n` +
      `방 이름: #{방이름}\n` +
      `입주 기간: #{입주일} ~ #{퇴실일}\n` +
  ...
  checkout_confirm_expired_guest: {
    ...
    varMap: {},
    // 게스트에게는 4-13(UH_0427)과 동일한 메시지 사용
    fallbackContent:
      `[보증금 반환 안내]\n\n` +
      `호스트님이 퇴실 확인을 완료하여 보증금 반환이 진행됩니다. (영업일 기준 3일 이내 지급 예정)`,
    buildFallbackSMS: () =>
  ...
  // 방 심사 승인 안내
  // =====================================================
  property_approved_host: {
    tplCode: 'UH_2739',
    targetRole: 'host',
    eventLabel: '방 심사 승인 안내_호스트',
    varMap: { roomName: '방이름' },
    fallbackContent:
      `[방 심사 승인 안내]\n\n` +
      `회원님께서 등록하신 방 #{방이름}의 심사가 승인되었습니다.`,
    buildFallbackSMS: (data) =>
      `[EZstay] 등록하신 방 '${data.roomName}'의 심사가 승인되었습니다.`
  },
  ...
  // =====================================================
  // 입주 준비 서비스 - 청소 결제 완료 (UI_1373, 임대인 수신)
  // 변수: 주소 / 청소날짜 / 청소시작시간 / 결제총액
  // =====================================================
  move_in_payment_completed_host: {
    tplCode: 'UI_1373',
    targetRole: 'host',
    eventLabel: '입주 준비 서비스 결제 완료_임대인',
    varMap: {
      address: '주소',
      cleaningDate: '청소날짜',
      cleaningStartTime: '청소시작시간',
      totalAmount: '결제총액'
    },
    fallbackContent:
      `[결제 완료 안내]\n` +
      `입주 준비 서비스 결제가 완료되어 안내드립니다.\n\n` +
      `주소 : #{주소}\n` +
      `청소 희망일 : #{청소날짜}\n` +
      `청소 시작 시간 : #{청소시작시간}\n` +
      `결제 금액: #{결제총액}원\n\n` +
      `청소 시작 시간 이후 최대 4시간 동안 청소 진행으로 입실이 어려울 수 있으니, 다른 임차인의 예약과 겹치지 않게 주의해주세요.`,
    buildFallbackSMS: (data) =>
      `[EZstay] 입주 준비 청소 결제 완료. 청소 희망일 ${data.cleaningDate} ${data.cleaningStartTime} / 금액 ${data.totalAmount}원`
  },

  // =====================================================
  // 입주 준비 서비스 - 옵션 결제 완료 (UI_1379, 임차인 수신)
  // 변수: 주소 / 입주일 / 옵션상품목록 / 결제총액
  // =====================================================
  move_in_payment_completed_guest: {
    tplCode: 'UI_1379',
    targetRole: 'guest',
    eventLabel: '입주 준비 서비스 결제 완료_임차인',
    varMap: {
      address: '주소',
      checkInDate: '입주일',
      optionItems: '옵션상품목록',
    ...
    fallbackContent:
      ...
      `상품: #{옵션상품목록}\n` +
      `결제 금액: #{결제총액}원\n\n` +
      `침구류 대여 상품을 결제하신 경우, 배송받은 침구 박스 및 비닐은 버리지 마시고 퇴실 시, 박스에 넣어서 반납해주세요.`,
    buildFallbackSMS: (data) =>
      `[EZstay] 입주 준비 옵션 결제 완료. 입주일 ${data.checkInDate} / 상품 ${data.optionItems} / 금액 ${data.totalAmount}원`
  },

  // =====================================================
  // 입주 준비 서비스 - 침구류 반납 안내 (UI_1355, 임차인 수신)
  // 변수 없음. 퇴실 당일 침구류 대여 결제자에게 발송.
  // =====================================================
  move_in_bedding_return_guest: {
    tplCode: 'UI_1355',
    targetRole: 'guest',
    eventLabel: '입주 준비 서비스 침구류 반납 안내_임차인',
    varMap: {},
    fallbackContent:
      `[퇴실 전 침구류 반납 안내]\n\n` +
      `퇴실 시, 대여하신 침구류는 포장 비닐에 넣은 후 침구류 전용 박스에 넣어서 문앞에 놓아주세요.\n\n` +
      `※ 주의사항\n` +
      ` - 반납 하지 않을 시, 불이익이 발생할 수 있습니다.\n` +
      ` - 임대인이 직접 제공한 침구류는 넣지 않도록 주의해주세요.`,
    buildFallbackSMS: () =>
      `[EZstay] 퇴실 시 대여하신 침구류는 포장 비닐에 넣어 전용 박스에 담아 문앞에 놓아주세요.`
  },

  // =====================================================
  // 입주 준비 서비스 - 결제 취소 (UI_1391, 공용: 임차인/임대인)
  // 변수: 주소 / 입주일 / 퇴실일 / 옵션상품목록 / 취소금액
  // =====================================================
  move_in_payment_canceled: {
    tplCode: 'UI_1391',
    targetRole: null,           // 공용 — 발송 시 receiverRole 로 구분
    eventLabel: '입주 준비 서비스 결제 취소_공용',
    varMap: {
      address: '주소',
      checkInDate: '입주일',
      checkOutDate: '퇴실일',
    ...
    fallbackContent:
      `[결제 취소 완료 안내]\n\n` +
      `입주 준비 서비스 결제 취소가 완료되었습니다.\n\n` +
      `주소 : #{주소}\n` +
      `임대 기간: #{입주일}~#{퇴실일}\n` +
      `상품명: #{옵션상품목록}\n` +
      `취소 금액: #{취소금액}원\n\n` +
      `최종 환불은 결제수단에 따라 5영업일 까지 소요될 수 있습니다.`,
    buildFallbackSMS: (data) =>
      `[EZstay] 입주 준비 서비스 결제 취소 완료. 상품 ${data.optionItems} / 취소 금액 ${data.cancelAmount}원`
  },
