...
/**
 * RentalPayment 모델 - 렌탈 주문 결제 정보
 * 토스페이먼츠로부터 받은 렌탈 결제 데이터를 저장합니다.
 */
module.exports = (sequelize) => {
  const RentalPayment = sequelize.define('RentalPayment', {
    ...
    orderId: {
      ...
      allowNull: false,
      comment: '주문번호 (RentalOrder의 orderId와 동일)'
    },
    method: {
    ...
    indexes: [
      {
        name: 'idx_rental_payment_rental_order_id',
        fields: ['rentalOrderId']
      },
      {
        name: 'idx_rental_payment_contract_id',
        fields: ['contractId']
      },
      {
        name: 'idx_rental_payment_key',
        unique: true,
        fields: ['paymentKey']
      },
      {
        name: 'idx_rental_payment_order_id',
        fields: ['orderId']
      },
      {
        name: 'idx_rental_payment_status',
        fields: ['status']
      }
    ]
  });
